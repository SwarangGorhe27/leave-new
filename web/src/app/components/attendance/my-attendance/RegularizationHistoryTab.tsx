...
interface RegularizationHistoryTabProps {
  requests: AttendanceRequest[];
  employeeId?: string;
}

export function RegularizationHistoryTab({
  requests,
  employeeId,
}: RegularizationHistoryTabProps) {
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState<"all" | "pending" | "approved" | "rejected">("all");
  const [dateRangeFilter, setDateRangeFilter] = useState<"all" | "this-month" | "last-month">("all");

  const getRequestDate = (request: AttendanceRequest): Date | null => {
    const candidate = request.attendanceDate || request.date || request.submittedDate;
    if (!candidate) return null;
    const normalized = candidate.includes("T") || candidate.includes(" ")
      ? candidate.replace(" ", "T")
      : candidate;
  ...
  const filteredRequests = useMemo(() => {
    ...
    if (dateRangeFilter !== "all") {
      ...
      filtered = filtered.filter((r) => {
        const requestDate = getRequestDate(r);
        if (!requestDate) return false;
        if (dateRangeFilter === "this-month") {
          return requestDate >= monthStart && requestDate <= now;
    ...
    return filtered.sort((a, b) => {
      const dateA = a.lastUpdated ? parseISO(a.lastUpdated.replace(" ", "T")) : new Date();
      const dateB = b.lastUpdated ? parseISO(b.lastUpdated.replace(" ", "T")) : new Date();
      return dateB.getTime() - dateA.getTime();
    });
  }, [requests, employeeId, searchTerm, statusFilter, dateRangeFilter]);
      ...
      <div className="space-y-2">
        {filteredRequests.length > 0 ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-xs font-semibold text-foreground dark:text-white">
                        {request.attendanceDate ? format(parseISO(request.attendanceDate), "MMM d, yyyy") : "N/A"}
                      </p>
                    ...
                    <p className="text-[11px] text-muted-foreground dark:text-slate-400 mt-1 line-clamp-2">
                      {request.reason}
                    </p>
                  </div>
                </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-xs text-muted-foreground dark:text-slate-400 mt-1">Your regularization history will appear here</p>
          </div>
        )}
      </div>
    </div>
